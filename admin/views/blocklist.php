    ...
    <form method="post">
        ...
        <table class="form-table">
            <tr>
                ...
                <td>
                    <input type="text" name="mshield_block_new_ip" value="" class="regular-text" placeholder="e.g., 192.168.1.1 or 10.0.0.0/8" />
...
<div class="mshield-section">
    <h2><?php esc_html_e( 'Blocked IPs', 'mighty-shield' ); ?></h2>
    <?php if( empty( $blocklist ) ) : ?>
        <p><?php esc_html_e( 'No IP addresses have been blocked yet.', 'mighty-shield' ); ?></p>
    <?php else : ?>
        <table class="mshield-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?php echo esc_html( ! empty( $entry['reason'] ) ? $entry['reason'] : '—' ); ?></td>
                    <td><?php echo esc_html( ! empty( $entry['added'] ) ? date_i18n( get_option( 'date_format' ), $entry['added'] ) : '—' ); ?></td>
                    <td>
                        <?php
                        $remove_url = wp_nonce_url(
                            admin_url( 'admin.php?page=mighty-shield&tab=blocklist&mshield_block_remove_ip=' . urlencode( $entry['ip'] ) ),
                            'mshield_block_remove_ip'
                        );
                        ?>
                        <a href="<?php echo esc_url( $remove_url ); ?>" class="button button-small" onclick="return confirm('<?php esc_attr_e( 'Remove this IP from the blocklist?', 'mighty-shield' ); ?>');"><?php esc_html_e( 'Remove', 'mighty-shield' ); ?></a>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
