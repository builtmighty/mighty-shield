...
<div class="mshield-docs">
    ...
    <main class="mshield-docs-main">
        ...
        <ol>
            ...
            <li><strong><?php esc_html_e( 'After payment.', 'mighty-shield' ); ?></strong> <?php esc_html_e( 'Your processor is the only thing that knows whether the billing address and the security code matched the card, and it answers once the payment has gone through. Those answers are checks like any other: they cost trust, they are added to what the checkout already found, and the rating is worked out again. Nothing can be refused by then, so an order that falls far enough is held before it ships instead.', 'mighty-shield' ); ?></li>
            <li><strong><?php esc_html_e( 'Afterwards.', 'mighty-shield' ); ?></strong> <?php esc_html_e( 'How the order turned out is remembered against everyone connected to it, and counts towards the next one. A checkout that was refused is remembered too, so somebody turned away does not arrive at their next attempt looking like a stranger.', 'mighty-shield' ); ?></li>
        </ol>
        ...
        <table>
            ...
            <tbody>
                <?php
                /* Computed, not typed. Every one of these bounds is a setting a
                   merchant can change, and printing them as literals meant the
                   manual started lying the moment anybody touched a threshold.
                   The Scoring tab fixed exactly this bug by reading spans();
                   this table was still hard-coded. */
                $mshield_span = \MightyShield\Includes\trust_badge::spans();

                $mshield_rows = [
                    'trusted' => [ __( 'Goes through untouched.', 'mighty-shield' ), __( 'Yes', 'mighty-shield' ), false ],
                    'low' => [ __( 'Goes through and is marked for you to look at. The customer notices nothing.', 'mighty-shield' ), __( 'Yes', 'mighty-shield' ), false ],
                    'elevated' => [ __( 'The customer confirms the payment with their bank.', 'mighty-shield' ), __( 'Yes', 'mighty-shield' ), false ],
                    'high' => [ __( 'The order is created and held with no payment attempted.', 'mighty-shield' ), __( 'No', 'mighty-shield' ), true ],
                    'rejected' => [ __( 'Refused at the checkout. No order is created.', 'mighty-shield' ), __( 'No', 'mighty-shield' ), true ],
                ];

                foreach( $mshield_rows as $mshield_key => $mshield_row ) :

                    $mshield_range = isset( $mshield_span[ $mshield_key ] )
                        ? sprintf( '%d&ndash;%d', $mshield_span[ $mshield_key ][0], $mshield_span[ $mshield_key ][1] )
                        : esc_html__( 'n/a', 'mighty-shield' );
                    ?>
                    <tr>
                        <td><?php echo wp_kses( $mshield_range, [] ) ?: esc_html( $mshield_range ); ?></td>
                        <td><span class="field"><?php echo esc_html( \MightyShield\Includes\risk_levels::label( $mshield_key ) ); ?></span></td>
                        <td><?php echo esc_html( $mshield_row[0] ); ?></td>
        ...
        <p>
            ...
            printf(
                ...
                esc_html__( 'The quickest version of all of this is the %s, which walks through the four decisions that matter and explains each one as it goes. It is safe to run again at any time: nothing is changed until you choose it.', 'mighty-shield' ),
                '<a href="' . esc_url( \MightyShield\Admin\setup_wizard::url() ) . '">' . esc_html__( 'setup walkthrough', 'mighty-shield' ) . '</a>'
            );
            ?>
        </p>
        ...
        <ol>
            ...
            <li><?php printf( wp_kses_post( __( 'Wait a week or two, then look at <a href="%s">Shielding</a>. It shows how many orders landed at each risk level and how they turned out.', 'mighty-shield' ) ), esc_url( $blocking_url ) ); ?></li>
        ...
        <p><?php esc_html_e( 'Each risk level is set to one action. These are all of them, with what each one does to the customer\'s money. Reject comes first because it is the one that stops the order happening at all; the rest run from the gentlest to the most disruptive.', 'mighty-shield' ); ?></p>
        <table>
        ...
        <h3><?php esc_html_e( 'How the checks are grouped', 'mighty-shield' ); ?></h3>
        <p><?php esc_html_e( 'The table is divided into six groups, by what a check is actually looking at. The grouping is there so you can reason about a whole category at once — if your customers keep tripping the Network checks, that is a different problem from tripping the Identity ones.', 'mighty-shield' ); ?></p>
        <ul>
            <li><strong><?php esc_html_e( 'Identity.', 'mighty-shield' ); ?></strong> <?php esc_html_e( 'Who the order says it is from: the email address, the delivery address, whether the postcode matches the state, whether the address exists at all, and how many other buyers have used it.', 'mighty-shield' ); ?></li>
            <li><strong><?php esc_html_e( 'Network.', 'mighty-shield' ); ?></strong> <?php esc_html_e( 'Where it came from: your blocklist, temporary blocks, whether the connection is a data centre or a VPN, and whether it resolves anywhere near the delivery address.', 'mighty-shield' ); ?></li>
            <li><strong><?php esc_html_e( 'Behavior.', 'mighty-shield' ); ?></strong> <?php esc_html_e( 'How the checkout was filled in: the hidden trap field, how long it took, what the browser says about itself, whether anything was typed or clicked, and the bot challenge.', 'mighty-shield' ); ?></li>
            <li><strong><?php esc_html_e( 'Order.', 'mighty-shield' ); ?></strong> <?php esc_html_e( 'What is being bought, and for how much.', 'mighty-shield' ); ?></li>
            <li><strong><?php esc_html_e( 'Payment.', 'mighty-shield' ); ?></strong> <?php esc_html_e( 'What your processor says about the card once the payment has gone through. See the note below, because these behave differently from the rest.', 'mighty-shield' ); ?></li>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr><td><span class="field"><?php esc_html_e( 'Checkout attempts per hour', 'mighty-shield' ); ?></span></td><td><?php esc_html_e( 'How many checkout attempts one address may make in the window before it is rate limited.', 'mighty-shield' ); ?></td><td class="default"><?php esc_html_e( '20 per hour', 'mighty-shield' ); ?></td></tr>
        ...
        <div class="callout">
            <span class="callout-label"><?php esc_html_e( 'The Payment checks answer late', 'mighty-shield' ); ?></span>
            <p><?php esc_html_e( 'The checks in the Payment group — whether the billing address matched the card, whether the security code matched, whether it was a prepaid card, what your processor\'s own fraud scoring said — are the only ones that cannot run at checkout. Your payment processor is the only thing that knows the answers, and it does not answer until the payment has gone through.', 'mighty-shield' ); ?></p>
            <p><?php esc_html_e( 'They are ordinary checks in every other way: they have a trust cost you set here, and it is added to what the checkout already found rather than judged on its own. What differs is what can be done about the result. The money has moved by then, so an order that falls far enough is held before it ships rather than refused. A successful charge that failed the address and the security code is one of the strongest signs of a stolen card there is, and it costs you nothing to stop the parcel.', 'mighty-shield' ); ?></p>
        </div>
        <div class="callout">
        ...
        <table>
            ...
            <tbody>
                ...
                <tr><td><span class="field"><?php esc_html_e( 'Refused', 'mighty-shield' ); ?></span></td><td><?php esc_html_e( 'MightyShield turned the checkout away', 'mighty-shield' ); ?></td><td><?php esc_html_e( 'A mark against, lighter than one of yours. Three of them is enough for the next attempt to be treated as a known bad address.', 'mighty-shield' ); ?></td></tr>
            </tbody>
        </table>
        <div class="callout">
            <span class="callout-label"><?php esc_html_e( 'Your verdict outranks everything', 'mighty-shield' ); ?></span>
            <p><?php esc_html_e( 'Outcomes only ever get worse: an order recorded as a chargeback will not quietly go back to being fine. Your own Clean or Fraud verdict is the exception. It can be changed as often as you like, in either direction, and changing it takes back whatever the previous verdict did.', 'mighty-shield' ); ?></p>
        ...
        <ol>
            ...
            <li><?php printf( wp_kses_post( __( 'On <a href="%s">Shielding</a>, raise the Rejected threshold so more orders are refused outright, and set Elevated to something firmer than 3-D Secure if your processor cannot do it.', 'mighty-shield' ) ), esc_url( $blocking_url ) ); ?></li>
            ...
            <li><?php printf( wp_kses_post( __( 'Turn on the bot challenge on <a href="%s">Shielding</a> if you have not. It is the single most effective thing against automated checkout traffic.', 'mighty-shield' ) ), esc_url( $blocking_url ) ); ?></li>
