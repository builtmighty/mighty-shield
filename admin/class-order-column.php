...
class order_column {
    ...
    public static function cell( $order ) {
        ...
        // No data-theme. A borrowed token scope inside somebody else's page
        // follows that page, not MightyShield's own preference: with Dark
        // selected this painted near-white text onto WordPress's white
        // background, and switched the severity ramp to its light-on-dark inks.
        // The order panel keeps its data-theme because .mshield-app paints its
        // own surface, so dark there is coherent.
        $open = '<span class="mshield-scope mshield-rating %s"%s>';
