...
class order_panel {
    ...
    private function render_verdict( $order ) {
        ...
        printf(
            ...
            esc_html( $current === ''
                ? __( 'Telling MightyShield how this order turned out is what teaches it. It weighs your answer against future orders from the same customer, address, card or network.', 'mighty-shield' )
     ...
     * @return  string
     */
    public static function action_url( $order, $do, $return = '', $paged = 0 ) {
        ...
        // Its own argument. The queue used to fold the page into $return as
        // "queue&paged=2", which add_query_arg then encoded to "queue%26paged%3D2"
        // -- and the check on the way back is an exact match against "queue".
        // So from page two onward the merchant was dropped on the single-order
        // screen, and the result message went to a transient only that screen
        // drains, where they would not see it until they navigated back.
        if( (int) $paged > 1 ) $args['ms_paged'] = (int) $paged;
    ...
    private function verdict( $order, $verdict ) {
        ...
        return [ $verdict === 'fraud'
            ? __( 'Marked as fraud. MightyShield will weigh this against future orders from the same customer, address, card or network.', 'mighty-shield' )
    ...
    private function block( $order ) {
        ...
        if( ai_capture::is_authorized( $order ) ) {
            ...
            // Unconfirmed is not a refusal, but it is not a release either, so
            // the order stays where it is. Cancelling an order whose funds may
            // still be reserved helps nobody: it reads as finished, the money is
            // not back with the customer, and nothing on the screen says to go
            // and look. Leaving it on hold keeps it in the queue until somebody
            // has checked the processor.
            if( ! $confirmed ) {

                $order->add_order_note( 'MightyShield: ' . __( 'Block attempted. The release was sent to the payment processor, which did not report back whether it completed, so the order has been left on hold. Check the authorization in your processor dashboard, then block it again or cancel it by hand.', 'mighty-shield' ) );
                $order->save();
                ...
                return [ __( 'The release was sent to your payment processor, but it did not confirm the authorization was cancelled. The order has been left on hold so it stays in your queue. Check it in your processor dashboard.', 'mighty-shield' ), 'warning' ];

            }

            $order->add_order_note( 'MightyShield: ' . __( 'Blocked in review. The authorization was released.', 'mighty-shield' ) );
            $order->save();
