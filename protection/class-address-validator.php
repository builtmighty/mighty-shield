...
class address_validator {
    ...
    /**
     * Score the billing/shipping address for suspicious patterns.
     *
     * This used to refuse at the threshold and say nothing below it. Now the
     * score IS the output: assess() emits address_fake with confidence scaled
     * to how far the score got, so an address at the threshold costs the full
     * weight — 80, which rejects on its own, exactly where the old cliff was —
     * and one at half the threshold costs half, which is a ramp rather than a
     * cliff and is the whole reason scoring exists.
     *
     * @since   1.0.0
     *
    ...
    public function validate_address( $data, $errors ) {
        ...
        db::log_event( ip_utils::get_client_ip(), 'classic_checkout', 'flagged', $verdict['reason'] );
     ...
     *
     * Emission is the only thing this does. There used to be a `blockable` key
     * beside the score — score >= threshold — and two callers refused on it.
     * It was the last reader of the old all-or-nothing threshold, and it said
     * nothing the scaled confidence below does not say better.
     *
     * @since   2.0.0
     *
     * @param   array   $data   Normalised billing fields.
     * @return  array   [ 'score', 'threshold', 'signals', 'reason' ]
     */
