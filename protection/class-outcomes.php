...
class outcomes {
    ...
    public static function record( $order, $outcome ) {
        ...
        //
        // This guard used to sit behind another one: any order carrying a
        // human verdict returned false right here, before the ladder. It was
        // written against redelivered webhooks and it rejected first-time ones
        // identically -- and order_panel::approve() writes _mshield_review on
        // every one of its exit paths. So every order a reviewer RELEASED from
        // the queue could never afterwards record a chargeback, a refund or
        // anything else. That is precisely the population where being wrong
        // matters: the orders the engine flagged and a human overruled, which
        // is also the only population the tuning report exists to measure.
        //
        // What the human verdict actually protects is their own -25 or +1, and
        // reversing that is what the ladder below already does correctly.
        ...
        // Take the previous outcome back off before applying the worse one.
        //
        // Only set_manual() used to do this, so the AUTOMATIC ladder stacked:
        // an order that completed and was then charged back left
        // approved_count = 1 AND chargeback_count = 1 on every identity, with
        // reputation at +1-100 rather than -100. The two most severe signals
        // branch on those counters rather than on reputation, so the stale
    ...
    public function bulk_action_notice() {
        ...
        printf(
            ...
            esc_html( sprintf(
                ...
                _n(
                    '%d order reported as fraud. MightyShield will weigh this against future orders from the same customer, address, card or network.',
