...
class response {
    ...
    /**
     * Whether a check is allowed to turn a customer away right now.
     *
     * Observe mode is supposed to mean the store watches and records and changes
     * nothing. It did not: the scoring ladder honoured it, and a dozen older
     * checks refused people directly without ever asking -- so a merchant told
     * "ratings are being recorded, not enforced" was losing orders to layers
     * that had never heard of the setting.
     *
     * Each of those layers still records its signal and its log row in observe
     * mode. Only the refusal waits. That is what makes observing worth doing:
     * the merchant sees exactly what would have happened before it happens to
     * anybody.
     *
     * Two layers deliberately do NOT consult this, and both are documented where
     * they live: the IP blocklist, because a merchant curating a list is giving
     * an instruction rather than asking for a rating; and the account-form
     * challenge on login, registration and password reset, because those are not
     * orders and observe mode is defined in terms of orders.
     *
     * @since   2.1.1
     *
     * @return  bool
     */
    public static function may_refuse() {

        return self::is_enforcing();
    ...
    public static function dispatch( $order, $action, $reason = '' ) {
        ...
        switch( $action ) {
            ...
            case actions::FLAG:
                ...
                // An order that tripped nothing is not worth a note.
                //
                // An unknown customer with a spotless order lands on exactly
                // 94.0 -- the ceiling that stops a first-timer being called
                // Trusted -- and 94 is inside Low, whose default action is to
                // flag. So every first order got "Flagged by the risk rating"
                // with no reasons under it, and a row in the review queue,
                // which is how the genuinely held orders get lost in it.
                //
            ...
            case actions::REJECT:
                ...
                // Reject means "no order", and by the time dispatch runs there
                // is one. Every path that can resolve to reject asks
                // response::refuses() first, at validation, and turns the
                // checkout away there -- so arriving here means the order was
                // created anyway: enforcement was off when validation ran and
                // on by the time this did, or a third party created the order
                // outside the checkout flow.
                //
                // Holding is the only honest answer. Letting it through ignores
                // the merchant's most severe setting; cancelling it here would
                // leave a customer who may already have been charged with a
                // dead order and no explanation. Held, the money is untouched
                // and a human decides.
                $order->add_order_note( 'MightyShield: ' . __( 'This order was set to be refused before it was created, but it already existed by the time the decision ran. It has been held for review instead, and nothing has been charged.', 'mighty-shield' ) );
