...
class risk_levels {
    ...
    /**
     * Levels the merchant can send to AI review.
     *
     * Deliberately NOT the same list as CONFIGURABLE, which is about what a
     * level does. This is about which orders are worth buying an opinion on,
     * and the two answers differ at exactly one level.
     *
     * Rejected is here and its action is not configurable. Under the old
     * ordering it could not have been: the review ran after the order existed,
     * and a rejected checkout never got that far. The review now runs at
     * validation, BEFORE the refusal, so a merchant who would rather not turn
     * anybody away on arithmetic alone can put a model in front of the decision
     * — and with the rating effect set to "lower or raise", a model that
     * recognises an ordinary customer can rescue the order.
     *
     * Banned is absent, and that is not squeamishness. It is reachable only
     * through a signal floor, never by losing trust, and the AI only moves the
     * trust number — evaluate() takes the more severe of the floor and the
     * score, so a floored verdict cannot be argued with. Offering it would sell
     * a call that provably cannot change anything.
     *
     * @since   2.2.0
     */
    const AI_REVIEWABLE = [ self::TRUSTED, self::LOW, self::ELEVATED, self::HIGH, self::REJECTED ];
    ...
    /**
     * What the merchant has CHOSEN for this level, regardless of whether AI can
     * run right now.
     *
     * ai_review() answers a different question -- "should this order be sent for
     * review" -- and correctly says no when there is no provider configured.
     * That makes it the wrong thing to render a form control from: the Blocking
     * tab would draw everybody's preference as off whenever AI was off, and
     * saving the page would then write that back as fact.
     *
     * @since   2.1.1
     *
     * @param   string  $level
     * @return  bool
     */
    public static function ai_review_setting( $level ) {

        if( ! isset( self::LADDER[ $level ] ) ) return false;
        if( ! \in_array( $level, self::AI_REVIEWABLE, true ) ) return false;
