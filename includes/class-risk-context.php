 ...
 * Risk context.
 *
 * The per-request signal bus. Every detector emits into this rather than
 * deciding anything itself, so that findings compound: three weak signals add
 * up to one strong verdict instead of producing three order notes nobody reads.
 *
 * Emitting is the whole job. Eight layers used to carry their own refuse-or-flag
 * setting alongside this, which made each of them a second decision engine --
 * and worse, that setting governed WHEN the layer emitted, so the context this
 * class handed to the ladder was only ever half-built at the moment the ladder
 * had to use it.
 *
...
class risk_context {
    ...
    /**
     * Rebuild the context from a stored signal array.
     *
     * The mirror of to_array(), and the reason the card pass can work at all.
     * Card details arrive from the processor after payment, on a webhook where
     * this class is empty — so scoring them there would judge the order on its
     * card alone and discard everything the checkout knew. db::save_risk()
     * stored the whole signal set as JSON; this puts it back so the card
     * signals are added to a complete picture rather than replacing it.
     *
     * Floors are re-derived from signals::CATALOG rather than read from the
     * stored row. The catalogue is the source of truth for what a signal means,
     * and a floor that was changed since the order was placed should apply as
     * it is now, not as it was.
     *
     * Additive on purpose: it does not clear first. A caller wanting a clean
     * slate calls reset(), which is explicit about it.
     *
     * @since   2.2.0
     *
     * @param   array   $signals    As produced by to_array()['signals'].
     * @return  int     Signals restored.
     */
    public static function restore( $signals ) {

        if( ! is_array( $signals ) ) return 0;

        $count = 0;

        foreach( $signals as $signal ) {

            if( ! is_array( $signal ) || empty( $signal['key'] ) ) continue;

            $key = (string) $signal['key'];

            // Goes through add(), so a signal that has since been switched off
            // or removed from the catalogue does not come back from the dead.
            if( self::add(
                $key,
                (string) ( $signal['reason'] ?? '' ),
                isset( $signal['confidence'] ) ? (float) $signal['confidence'] : 1.0
            ) ) {
                $count++;
            }
