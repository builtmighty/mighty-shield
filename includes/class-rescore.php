...
class rescore {
    ...
    const REPLAYABLE = [
        ...
        'entity_linked_bad',
        'entity_trusted',
        // The card. Every field the processor sent is on the order as meta, so
        // unlike the bot, timing and device layers this one really can be
        // replayed -- it is a stored fact about the order rather than a
        // measurement of the request that placed it.
        'card_avs_fail',
        'card_cvc_fail',
        'card_processor_risk',
    ...
    private static function collect( $order, $use_ai ) {
        ...
        // The card, when a processor answered for this order. Replayed through
        // card_signals::emit() rather than re-judged here, so a re-rate and a
        // checkout reach the same conclusion from the same code.
        if( ! class_exists( '\\MightyShield\\Protection\\card_signals' ) ) {
            require_once MSHIELD_PATH . 'protection/class-card-signals.php';
        }

        $card = \MightyShield\Protection\card_signals::stored( $order );
        if( ! empty( $card ) ) \MightyShield\Protection\card_signals::emit( $order, $card );
